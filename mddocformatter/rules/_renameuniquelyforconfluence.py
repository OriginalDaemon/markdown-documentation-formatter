...
def path_is_confluence_style(root_directory: Path, input_path: Path, version_name: str) -> bool:
    ...
    if len(rel_path.parts) > 1:
        return input_path.name == f"{rel_path.parts[-2]}.md" or \
               re.match(f"{rel_path.parts[-2]} - .+.md", input_path.name)
    else:
        return not version_name or re.match(f"{version_name} - .+.md", input_path.name)
