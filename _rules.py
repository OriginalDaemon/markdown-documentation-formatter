        ...
        if TABLE_OF_CONTENTS_VARIABLE in line:
            ...
            for remaining_line in map(lambda x: x.strip(), lines[i + 1 :]):
                if remaining_line.startswith("#"):
                    ...
                    indent_size = max(
                        0, len(remaining_line) - len(remaining_line.lstrip("#")) - 1
                    )
                    table.append(
                        "{} - {}".format(
                            "  " * indent_size,
                            format_markdown_link(stripped, "#{}".format(stripped)),
                        )
                    )
...
def move_to_target_dir_relative(context: ProcessingContext, document: Document):
    ...
    """
    if (
        context.settings.target_directory
        and context.settings.root_directory != context.settings.target_directory
    ):
