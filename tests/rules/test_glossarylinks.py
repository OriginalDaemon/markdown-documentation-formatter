...
class TestGlossaryLinks(unittest.TestCase):
    ...
    def test_link_term(self):
        ...
        doc = Document(
            Path(context.settings.root_directory / "test" / "test.md"), "Here is an Example of something."
        )
    ...
    def test_link_synonym(self):
        context = self._createContext()
        doc = Document(
            Path(context.settings.root_directory / "test" / "test.md"), "Here is a demo of something."
        )
        rules.add_glossary_links(context, doc)
    ...
    def test_link_phrase(self):
        context = self._createContext()
        doc = Document(
            Path(context.settings.root_directory / "test" / "test.md"), "Here is a Test Term."
        )
        rules.add_glossary_links(context, doc)
